...
try {
    ...
    res.status(201).json({user:user._id,created:true,token}) // send token to front end
} catch(err) {
    console.log("ERR")
...
module.exports.login = async(req, res, next) => {
    try {
        ...
        res.status(200).json({user:user._id,created:true,token}) // send token to front end
